...
def change_master_password():
        ...
        if master_password != "":
            master_password_input = getpass("Enter the current master password: ")
            while master_password_input != master_password:
                print("Incorrect master password.")
                master_password_input = getpass("Enter the current master password again: ")
            changed_master_password = getpass("Enter the new master password: ")
            while len(changed_master_password) > 20:
                print("Master password must be less than 20 characters.")
                changed_master_password = getpass("Enter the new master password: ")
            else :
                master_password = changed_master_password
...
def verify_master_password():
    global master_password
    if master_password != "":
        master_password_input = getpass("Enter the master password: ")
        while master_password_input != master_password:
            print("Incorrect master password.")
            master_password_input = getpass("Enter the master password again: ")
...
def check_password_strength():
    flag = False
    print("You chose to check password strength.")
    ...
    if len(password_to_check) < 8:
        ...
        flag = False
    elif not any(char.isdigit() for char in password_to_check):
        print("Weak password: Password should contain at least one digit.")
        flag = False
    elif not any(char.isupper() for char in password_to_check):
        print("Weak password: Password should contain at least one uppercase letter.")
        flag = False
        ...
        flag = True
    while flag == False:
        password_to_check = input("Enter the password you want to check again: ")
        if len(password_to_check) < 8:
            print("Weak password: Password should be at least 8 characters long.")
            flag = False
        elif not any(char.isdigit() for char in password_to_check):
